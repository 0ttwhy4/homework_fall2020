...
class RL_Trainer(object):
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        for itr in range(n_iter):
            ...
            if self.log_video or self.log_metrics:
                ...
                # perform logging
                self.logger.text_logger.debug('Beginning logging procedure...')
                self.perform_logging(
                    itr, paths, eval_policy, train_video_paths, training_logs)

                if self.params['save_params']:
                    self.logger.text_logger.debug('Saving agent params')
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, training_logs):
        ...
        if self.log_video and train_video_paths != None:
            self.logger.text_logger.debug('Collecting video rollouts eval')
            eval_video_paths = utils.sample_n_trajectories(self.env, eval_policy, MAX_NVIDEO, MAX_VIDEO_LEN, True)

            #save train/eval videos
            self.logger.text_logger.debug('Saving train rollouts as videos...')
